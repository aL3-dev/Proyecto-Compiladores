# ============================================================
# Compilador PseudoC - Archivo unificado
# Fases: Léxico → Sintáctico → Código Intermedio
# ============================================================

# Listas compartidas entre las tres fases
all_tokens  = []
all_lexemas = []
all_lin     = []

# ============================================================
# FUNCIONES COMPARTIDAS (Sintáctico y Código Intermedio)
# ============================================================

def actual(pos):
    return all_tokens[pos] if pos < len(all_tokens) else -1

def avanzar(pos):
    return pos + 1 if pos < len(all_tokens) else pos

def verificar(pos, t, desc="", error_fn=None):
    if actual(pos) == t:
        return avanzar(pos)
    if error_fn:
        error_fn("Se esperaba " + desc)
    return pos
...
def estados():
    ...
    if estado == 1:                       # identificador / reservada
        if c.isalpha() or c.isdigit(): return 1
        return 3                          # acepta directo (elimina estado 2)
    ...
    if estado == 5:                       # numero, despues del punto
        ...
        if lex.endswith("."): return 33
        return 6

    if estado == 7:                       # cadena de texto
        if c == "'":                              return 8
        if c.isalpha() or c.isdigit() or c == " ": return 7
        return 33
    ...
    if estado == 10: return 11            # se leyo '++'

    if estado == 15:
        if c == "=": return 16
        return 17
    if estado == 16: return 17

    if estado == 18:
        if c == "=": return 19
        return 20
    if estado == 19: return 20

    if estado == 21:
        if c == "=": return 22
        return 23
    if estado == 22: return 23

    return estado


def tokenizar(cadena_linea, num_linea):
    global cadena, i, estado, lex, col_inicio
    cadena  = cadena_linea + "$"
    i       = 0
    estado  = 0
    lex     = ""
    ...
    while i < len(cadena) - 1 or estado != 0:
        ...
        if estado == 0:
            ...
            if estado == 33:
                msg = ("'!' no es valido sin '=' a continuacion"
                       if c == "!" else "caracter no reconocido '" + c + "'")
                errores.append("Linea " + str(num_linea) + ", columna " +
                               str(col_inicio + 1) + ": " + msg)
                tokens.append(53); lexemas.append(c)
                i += 1; estado = 0
                continue
            if estado == 7:
                i += 1
        ...
        if estado == 3:                   # ACEPTACION: id o reservada
            r = es_reservada(lex)
        ...
        if estado == 4:
            ...
            if sig == 33:
                ...
                tokens.append(53); lexemas.append(lex)
                while cadena[i].isdigit(): i += 1
                estado = 0; continue
            if sig == 5:
                lex += cadena[i]; i += 1
                estado = 5; continue
        ...
        if estado == 6:                   # ACEPTACION: numero
            tokens.append(2); lexemas.append(lex)
            estado = 0; continue

        if estado == 7:
            sig = estados()
            if sig == 7:
                lex += cadena[i]; i += 1; continue
            if sig == 8:
                estado = 8; continue
            if cadena[i] == "$":
                errores.append("Linea " + str(num_linea) + ", columna " +
        ...
        if estado == 8:                   # ACEPTACION: cadena de texto
            i += 1
            tokens.append(1); lexemas.append(lex)
            estado = 0; continue

        if estado == 9:
            lex += cadena[i]; i += 1
            estado = estados(); continue

        if estado == 10:
            lex += cadena[i]; i += 1
            estado = 11; continue

        if estado == 11:                  # ACEPTACION: + o ++
            tokens.append(52 if lex == "++" else 37)
            lexemas.append(lex)
            estado = 0; continue
        ...
        if estado == 17:                  # ACEPTACION: < o <=
            tokens.append(43 if lex == "<=" else 41)
            lexemas.append(lex)
            estado = 0; continue
        ...
        if estado == 20:                  # ACEPTACION: > o >=
            tokens.append(44 if lex == ">=" else 42)
            lexemas.append(lex)
            estado = 0; continue
        ...
        if estado == 23:                  # ACEPTACION: = o ==
            tokens.append(45 if lex == "==" else 47)
            lexemas.append(lex)
            estado = 0; continue

        if estado == 24:                  # ACEPTACION: !=
            tokens.append(46); lexemas.append("!=")
            i += 2; estado = 0; continue
...
def error_sint(msg):
    ...
    sint_e = True
    if sint_pos < sint_n:
        print("ERROR SINTACTICO -> linea", all_lin[sint_pos - 1], ":", msg)
    else:
...
def ver(t, desc=""):
    global sint_pos
    sint_pos = verificar(sint_pos, t, desc, error_sint)
...
def lista_sentencias():
    ...
    if t in (22, 23, 24, 1000, 33, 34):
        ...
        ciclo(); lista_sentencias()
    elif t in (21, 31, -1):
        return
    else:
        error_sint("Token inesperado")
        sint_pos = avanzar(sint_pos)
        lista_sentencias()


def sentencia_simple():
    t = actual(sint_pos)
    if t in (22, 23, 24): declaracion()
    elif t == 1000:        asignacion()
    elif t in (33, 34):    io()
    else: error_sint("Se esperaba declaracion, asignacion o entrada/salida")
...
def tipo():
    if actual(sint_pos) not in (22, 23, 24):
        error_sint("Se esperaba tipo (entero, real, cadena)")
    ver(actual(sint_pos), "tipo")


def lista_variables():
    ver(1000, "variable")
    if actual(sint_pos) == 48:
        ver(48, "','"); lista_variables()


def asignacion():
    ver(1000, "variable"); ver(47, "'='"); expresion()
...
def io():
    ...
    if t == 33:
        ver(33, "'leer'"); ver(50, "'('")
        ver(1000, "variable"); ver(51, "')'")
    elif t == 34:
        ver(34, "'mostrar'"); ver(50, "'('")
        contenido(); ver(51, "')'")
    else:
        error_sint("Se esperaba leer o mostrar")


def contenido():    exp_concat()


def exp_concat():
    elemento_concat()
    if actual(sint_pos) == 52:
        ver(52, "'++'"); exp_concat()


def elemento_concat():
    if actual(sint_pos) == 1: ver(1, "literal")
    else: expresion()


def condicional():
    ver(30, "'si'"); ver(50, "'('")
    exp_relacional(); ver(51, "')'")
    ver(20, "'inicio'"); lista_sentencias(); ver(21, "'fin'")
    while actual(sint_pos) == 31:
        ver(31, "'sino'")
        if actual(sint_pos) == 50:
            ver(50, "'('"); exp_relacional(); ver(51, "')'")
        ver(20, "'inicio'"); lista_sentencias(); ver(21, "'fin'")


def ciclo():
    ver(32, "'mientras'"); ver(50, "'('")
    exp_relacional(); ver(51, "')'")
    ver(20, "'inicio'"); lista_sentencias(); ver(21, "'fin'")
...
def expresion():
    termino()
    while actual(sint_pos) in (37, 38):
        ver(actual(sint_pos), "'+' o '-'"); termino()


def termino():
    factor()
    while actual(sint_pos) in (39, 40):
        ver(actual(sint_pos), "'*' o '/'"); factor()


def factor():
    t = actual(sint_pos)
    if t == 50:
        ver(50, "'('"); expresion(); ver(51, "')'")
    elif t == 1000: ver(1000, "variable")
    elif t == 2:    ver(2, "numero")
    elif t in (25, 26, 27, 28, 29): funcion()
    else: error_sint("Se esperaba factor")


def funcion():
    t = actual(sint_pos)
    if t in (25, 26, 27, 28):
        ver(t, "funcion"); ver(50, "'('")
        expresion(); ver(51, "')'")
    elif t == 29: ver(29, "'pi'")
    else: error_sint("Se esperaba funcion")


def exp_relacional():
    comparacion()
    while actual(sint_pos) in (35, 36):
        ver(actual(sint_pos), "'Y' o 'O'"); comparacion()


def comparacion():
    expresion()
    if actual(sint_pos) in (41, 42, 43, 44, 45, 46):
        ver(actual(sint_pos), "operador relacional"); expresion()
    else:
        error_sint("Se esperaba operador relacional")
